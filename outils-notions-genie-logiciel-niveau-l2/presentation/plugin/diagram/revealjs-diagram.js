(function (window) {
  ...
  function init(treeData) {
    width =
      window.parent.document.body.clientWidth - margin.right - margin.left;
    height =
      window.parent.document.body.clientHeight - margin.top - margin.bottom;
  ...
  function renderStandaloneLinks(links) {
    var link = svg.selectAll("path.link-alone").data(links, function (d) {
      return "standalone" + d.key;
    });
